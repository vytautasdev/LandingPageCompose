...
import com.vytautasdev.styles.LogoStyle
...
fun RightSide() {
    Row(
        modifier = Modifier.fillMaxWidth().borderRadius(r = 50.px).backgroundColor(Theme.LighterGray.rgb)
            .padding(all = 20.px), horizontalArrangement = Arrangement.End
    ) {
        Section.values().take(6).forEach { section ->
            Link(
                modifier = NavigationItemStyle.toModifier().padding(right = 30.px).fontFamily(FONT_FAMILY)
...
fun LeftSide(
    ...
    Row(
        ...
        Image(
            modifier = LogoStyle.toModifier(), src = Res.Image.logo, desc = "Logo Image"
        )
    }
}
